...
namespace gp2_proje
{
    public partial class masa1 : Form
    {
        ...
        void LoadData()
        {
            ...
            da.Fill(menutablo);
            menu_masa1.DataSource = menutablo;

            baglanti.Close();
        }

        void masa1_siapris_getir()
        {
            baglanti = new SqlConnection("Data Source=DESKTOP-NOTHH0Q\\SQLEXPRESS01; Initial Catalog=DB; Integrated Security=True");
            baglanti.Open();

            da = new SqlDataAdapter("SELECT masa1.urun_adi AS urun,masa1.urun_fiyat AS fiyat FROM masa1", baglanti);
            DataTable siparistablo = new DataTable();
            da.Fill(siparistablo);
            masa1siparis.DataSource = siparistablo;
        ...
        private void BaslangicMenuDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            ...
            DataGridViewSelectedRowCollection selectedRows = menu_masa1.SelectedRows;
        ...
        private void UrunuSiparisTablosunaEkle(DataGridViewRow selectedRow)
        {
            ...
            masa1siparis.Rows.Add(selectedRow.Cells["urun"].Value, selectedRow.Cells["fiyat"].Value);
        }

        private void TransferData()
        {
            // Seçilen satırı alın
            if (menu_masa1.SelectedRows.Count > 0)
            {
                DataGridViewRow selectedRow = menu_masa1.SelectedRows[0];
                ...
                string insertQuery = $"INSERT INTO masa1 (urun_adi, urun_fiyat) VALUES ('{urunAdi}', '{fiyat}')";
        ...
        private void iptal_Click(object sender, EventArgs e)
        {
            ...
            if (masa1siparis.SelectedRows.Count > 0)
            {
                int rowIndex = masa1siparis.SelectedRows[0].Index;

                // Seçili satırdan veri alın
                string urunAdi = masa1siparis.Rows[rowIndex].Cells["urun"].Value.ToString();
                string fiyat = masa1siparis.Rows[rowIndex].Cells["fiyat"].Value.ToString();

                // Veritabanındaki masa1 tablosundan da silmek için sorguyu oluşturun
                string deleteQuery = $"DELETE FROM masa1 WHERE urun_adi = '{urunAdi}' AND urun_fiyat = '{fiyat}'";
                ...
                masa1siparis.Rows.RemoveAt(rowIndex);
            }
            else
            {
        ...
        {
            masa1odeme masaodemeekranınagit = new masa1odeme();

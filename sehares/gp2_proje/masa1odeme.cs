...
namespace gp2_proje
{
    public partial class masa1odeme : Form
    {
        ...
        void masa1_odeme_getir()
        {
            baglanti = new SqlConnection("Data Source=DESKTOP-NOTHH0Q\\SQLEXPRESS01; Initial Catalog=DB; Integrated Security=True");
            baglanti.Open();

            da = new SqlDataAdapter("UPDATE masa1 SET urun_fiyat = urun_fiyat * 1.05  SELECT masa1.urun_adi AS urun,masa1.urun_fiyat AS fiyat FROM masa1", baglanti);
            DataTable siparistablo = new DataTable();
            da.Fill(siparistablo);

            masaodeme1.DataSource = siparistablo;
        ...
        void toplam_fiyat_getir()
        {
            ...
            da = new SqlDataAdapter("SELECT SUM(masa1.urun_fiyat) AS toplamfiyat FROM masa1", baglanti);
            DataTable siparistablo = new DataTable();
            da.Fill(siparistablo);
            masa1toplam.DataSource = siparistablo;
        ...
        private void ode_Click(object sender, EventArgs e)
        {
            ...
            if (masaodeme1.SelectedRows.Count > 0)
            {
                DataGridViewRow selectedRow = masaodeme1.SelectedRows[0];
                ...
                if (masaodeme1.SelectedRows.Count > 0)
                {
                    ...
                    int rowIndex = masaodeme1.SelectedRows[0].Index;

                    // Seçili satırdan veri alın
                    string urunAd = masaodeme1.Rows[rowIndex].Cells["urun"].Value.ToString();
                    string fiyat = masaodeme1.Rows[rowIndex].Cells["fiyat"].Value.ToString();

                    // Veritabanındaki masa1 tablosundan da silmek için sorguyu oluşturun
                    string deleteQuery = $"DELETE FROM masa1 WHERE urun_adi = '{urunAd}' AND urun_fiyat = '{fiyat}'";
                    ...
                    masaodeme1.Rows.RemoveAt(rowIndex);
                }
                else
                {
        ...
        private void masa1odeme_Load(object sender, EventArgs e)
        {
            masa1_odeme_getir();
            toplam_fiyat_getir();
        }

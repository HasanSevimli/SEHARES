...
namespace gp2_proje
{
    public partial class masa2odeme : Form
    {
        ...
        void masa_odeme_getir()
        {
            ...
            da = new SqlDataAdapter("UPDATE masa2 SET urun_fiyat = urun_fiyat * 1.05  SELECT masa2.urun_adi AS urun,masa2.urun_fiyat AS fiyat FROM masa2", baglanti);
            DataTable siparistablo = new DataTable();
            da.Fill(siparistablo);

            masaodeme2.DataSource = siparistablo;
        ...
        void toplam_fiyat_getir()
        {
            ...
            da = new SqlDataAdapter("SELECT SUM(masa2.urun_fiyat) AS toplamfiyat FROM masa2", baglanti);
            DataTable siparistablo = new DataTable();
            da.Fill(siparistablo);
            masa2toplam.DataSource = siparistablo;
        ...
        {
            if (masaodeme2.SelectedRows.Count > 0)
            {
                DataGridViewRow selectedRow = masaodeme2.SelectedRows[0];
                ...
                if (masaodeme2.SelectedRows.Count > 0)
                {
                    ...
                    int rowIndex = masaodeme2.SelectedRows[0].Index;

                    // Seçili satırdan veri alın
                    string urunAd = masaodeme2.Rows[rowIndex].Cells["urun"].Value.ToString();
                    string fiyat = masaodeme2.Rows[rowIndex].Cells["fiyat"].Value.ToString();

                    // Veritabanındaki masa1 tablosundan da silmek için sorguyu oluşturun
                    string deleteQuery = $"DELETE FROM masa2 WHERE urun_adi = '{urunAd}' AND urun_fiyat = '{fiyat}'";
                    ...
                    masaodeme2.Rows.RemoveAt(rowIndex);
                }
                else
                {

...
namespace gp2_proje
{
    public partial class masa5odeme : Form
    {
        ...
        private void masa5odeme_Load(object sender, EventArgs e)
        {
            masa_odeme_getir();
            toplam_fiyat_getir();
        }
        void masa_odeme_getir()
        {
            ...
            da = new SqlDataAdapter("UPDATE masa5 SET urun_fiyat = urun_fiyat * 1.05  SELECT masa5.urun_adi AS urun,masa5.urun_fiyat AS fiyat FROM masa5", baglanti);
        ...
        void toplam_fiyat_getir()
        {
            ...
            da = new SqlDataAdapter("SELECT SUM(masa5.urun_fiyat) AS toplamfiyat FROM masa5", baglanti);
            DataTable siparistablo = new DataTable();
            da.Fill(siparistablo);
        ...
        private void ode_Click(object sender, EventArgs e)
        {
            if (masaodeme.SelectedRows.Count > 0)
            {
                ...
                if (masaodeme.SelectedRows.Count > 0)
                {
                    ...
                    string deleteQuery = $"DELETE FROM masa5 WHERE urun_adi = '{urunAd}' AND urun_fiyat = '{fiyat}'";

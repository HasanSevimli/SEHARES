...
namespace gp2_proje
{
    public partial class masa6 : Form
    {
        ...
        private void masa6_Load(object sender, EventArgs e)
        {
            LoadData();
            masa6_siparis_getir();
        }
        void LoadData()
        {
            ...
            da.Fill(menutablo);
            menu_masa6.DataSource = menutablo;

            baglanti.Close();
        }
        void masa6_siparis_getir()
        {
            baglanti = new SqlConnection("Data Source=DESKTOP-NOTHH0Q\\SQLEXPRESS01; Initial Catalog=DB; Integrated Security=True");
            baglanti.Open();

            da = new SqlDataAdapter("SELECT masa6.urun_adi AS urun,masa6.urun_fiyat AS fiyat FROM masa6", baglanti);
            DataTable siparistablo = new DataTable();
            da.Fill(siparistablo);
            masa6siparis.DataSource = siparistablo;
        ...
        private void BaslangicMenuDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            ...
            DataGridViewSelectedRowCollection selectedRows = menu_masa6.SelectedRows;
        ...
        private void UrunuSiparisTablosunaEkle(DataGridViewRow selectedRow)
        {
            ...
            masa6siparis.Rows.Add(selectedRow.Cells["urun"].Value, selectedRow.Cells["fiyat"].Value);
        }
        private void TransferData()
        {
            // Seçilen satırı alın
            if (menu_masa6.SelectedRows.Count > 0)
            {
                DataGridViewRow selectedRow = menu_masa6.SelectedRows[0];
                ...
                string insertQuery = $"INSERT INTO masa6 (urun_adi, urun_fiyat) VALUES ('{urunAdi}', '{fiyat}')";
        ...
        private void iptal_Click(object sender, EventArgs e)
        {
            ...
            if (masa6siparis.SelectedRows.Count > 0)
            {
                int rowIndex = masa6siparis.SelectedRows[0].Index;

                // Seçili satırdan veri alın
                string urunAdi = masa6siparis.Rows[rowIndex].Cells["urun"].Value.ToString();
                string fiyat = masa6siparis.Rows[rowIndex].Cells["fiyat"].Value.ToString();

                // Veritabanındaki masa1 tablosundan da silmek için sorguyu oluşturun
                string deleteQuery = $"DELETE FROM masa6 WHERE urun_adi = '{urunAdi}' AND urun_fiyat = '{fiyat}'";
                ...
                masa6siparis.Rows.RemoveAt(rowIndex);
            }
            else
            {
        ...
        {
            masa6odeme masaodemeekranınagit = new masa6odeme();
